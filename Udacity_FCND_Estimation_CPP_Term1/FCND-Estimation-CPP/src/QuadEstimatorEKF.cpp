...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
    ...
    Quaternion<float> gyro_attitude = Quaternion<float>::FromEuler123_RPY(rollEst, pitchEst, ekfState(6));
    
    V3D eulerRPY = gyro_attitude.IntegrateBodyRate(gyro, dtIMU).ToEulerRPY();
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
    ...
    V3F accI = attitude.Rotate_BtoI(accel);
    
    predictedState(0) = curState(0) + curState(3) * dt ;
    predictedState(1) = curState(1) + curState(4) * dt ;
    predictedState(2) = curState(2) + curState(5) * dt ;
    
    predictedState(3) = curState(3) + accI.x * dt ;
    predictedState(4) = curState(4) + accI.y * dt;
    predictedState(5) = curState(5) + accI.z * dt - CONST_GRAVITY * dt ;
    /////////////////////////////// END STUDENT CODE ////////////////////////////
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
    ...
    ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    // Warm up version
    /* MatrixXf bMatrix(4, 7);
    bMatrix.setZero();
    
    //set Rbg
    bMatrix(3,0) = cos(rollEst)*cos(ekfState(6));
    bMatrix(3,1) = sin(pitchEst)*sin(rollEst)-cos(pitchEst)*sin(ekfState(6));
    bMatrix(3,2) = cos(pitchEst)*sin(rollEst)*cos(ekfState(6))+sin(pitchEst)*sin(ekfState(6));
    bMatrix(4,0) = cos(rollEst)*sin(ekfState(6));
    bMatrix(4,1) = sin(pitchEst)*sin(rollEst)+cos(pitchEst)*cos(ekfState(6));
    bMatrix(4,2) = cos(pitchEst)*sin(rollEst)-sin(pitchEst)*cos(ekfState(6));
    bMatrix(5,0) = -sin(rollEst);
    bMatrix(5,1) = cos(rollEst)*sin(ekfState(6));
    bMatrix(5,2) = cos(rollEst)*cos(ekfState(6));
    bMatrix(6,6) = 1.0;
    
    
    /// We are taking Jacobian
    VectorXf u(4);
    u(0) = accel.x*dt; // acceleration in  body frame
    u(1) = accel.y*dt; // acceleration in  body frame
    u(2) = accel.z*dt; // acceleration in  body frame
    u(3) = ekfState(6); // yaw in Global Frame
    
    MatrixXf bProductUdtMatrix = bMatrix * u;
    
    MatrixXf gMatrix = newState + bProductUdtMatrix;
    ...
    VectorXf RbgPrimeProductUtMatrix = RbgPrime * u3;
    gPrime(0,3) = dt;
    gPrime(1,4) = dt;
    gPrime(2,5) = dt;
    gPrime(3,6) = RbgPrimeProductUtMatrix(0);
    gPrime(4,6) = RbgPrimeProductUtMatrix(1);
    gPrime(5,6) = RbgPrimeProductUtMatrix(2);
    gPrime(5,6) = 1.0;
    
    //This is the "in place" version of transpose(): it replaces *this by its own transpose. Thus, doing by following . Could be performance consideration
...
void QuadEstimatorEKF::UpdateFromGPS(V3F pos, V3F vel)
{
    ...
    //set Hprime diagonal matrix value to 1 which is < 7
    for ( int i = 0; i < 6; i++) {
        hPrime(i,i) = 1;
    }
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
    ...
    /*float newYaw = ( magYaw +ekfState(6) )/R_Mag(0, 0);
    //float newYaw =  magYaw - ekfState(6) ;
    zFromX(0) = newYaw;
    // normalize yaw to -pi .. pi
